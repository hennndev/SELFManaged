    ...
    return (
        <div className='flexx'>
            {pathname.map((path, index) => (
                <Link href={`/${hrefPath(index)}`} key={path} className={`flexx hover:underline ${index == pathname.length - 1 ? 'text-blue-700 dark:text-blue-500' : 'text-gray-500 dark:text-gray-300'}`}>
